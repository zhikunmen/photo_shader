...
(function (SpecialEffects) {
    var V_SHADER = "attribute vec2 aVertexPosition;\n" +
        ...
        "   vColor = aColor;\n" +
        "}";
    var BRIGHTNESS_CONTRAST_F_SHADER = 'precision lowp float;' +
        'varying vec4 vColor;' +
    ...
    var DENOISE_F_SHADER = 'precision lowp float;' +
        ...
        'varying vec2 vTextureCoord;' +
        'void main(){' +
        'vec2 texSize = vec2(texSizeW,texSizeH);' +
        'vec4 center=texture2D(uSampler,vTextureCoord)*vColor;' +
        'vec4 color=vec4(0.0);' +
        'float total=0.0;' +
        'for(float x=-4.0;x<=4.0;x+=1.0){' +
        'for(float y=-4.0;y<=4.0;y+=1.0){' +
        'vec4 sample=texture2D(uSampler,vTextureCoord+vec2(x,y)/texSize)*vColor;' +
        ...
        '}';
    var NOISE_F_SHADER = 'precision lowp float;' +
        'varying vec2 vTextureCoord;' +
        'varying vec4 vColor;' +
        'uniform sampler2D uSampler;' +
        'uniform float amount;' +
